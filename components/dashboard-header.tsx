...
export function DashboardHeader() {
  ...
  const handleLogout = () => {
    logout();
    toast.info('Você saiu da sua conta. Redirecionando para a página de login...');
  ...
  if (isLoading) {
    return (
      <>
        ...
        <div className="fixed top-0 left-72 z-30 hidden md:block transition-all duration-300 peer-data-[state=collapsed]:left-24 mt-[6rem]">
          <svg width="50" height="50" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
            <path d="M0,100 V0 H100 A100,100 0 0 0 0,100" fill="#46142B" />
          </svg>
        </div>

        <header className="fixed top-0 z-40 h-24 flex items-center justify-between bg-background transition-all duration-300 w-full left-0 px-4 md:px-6 md:left-72 md:w-[calc(100%-18rem)] md:peer-data-[state=collapsed]:left-24 md:peer-data-[state=collapsed]:w-[calc(100%-6rem)]">
  ...
  if (!user) {
    return (
      <>
        ...
        <header className="fixed top-0 z-40 h-24 flex items-center justify-between bg-background transition-all duration-300 w-full left-0 px-4 md:px-6 md:left-72 md:w-[calc(100%-18rem)] md:peer-data-[state=collapsed]:left-24 md:peer-data-[state=collapsed]:w-[calc(100%-6rem)]"></header>
      </>
    );
  }
  ...
  return (
    <>
      ...
      <header className="fixed top-0 z-40 h-24 flex items-center justify-between bg-background transition-all duration-300 w-full left-0 px-4 md:px-6 md:left-72 md:w-[calc(100%-18rem)] md:peer-data-[state=collapsed]:left-24 md:peer-data-[state=collapsed]:w-[calc(100%-6rem)]">
        <div className="flex items-center space-x-4 ml-12 md:ml-0">
          <h1 className="text-lg md:text-xl font-semibold text-white"></h1>
        </div>
